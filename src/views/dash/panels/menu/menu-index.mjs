  ...
  async displayMenu(menus) {
    ...
    for (const [menuID, menu] of Object.entries(menus)) {
      ...
      if (!categories[menu.category] && menu.category !== '삭제된 메뉴') {
        ...
        radioButtons[
          menu.category
        ] += `<input type="radio" name="category" id="${menu.category}"`;
      ...
      menuElement.addEventListener('click', (event) => {
        ...
        window.location.href = `/dash/menu/${mid}`;
        /*
    ...
    // 위쪽 카테고리 버튼 동작!
    const selCates = document.querySelectorAll(
      '#category-group div input[type="radio"]'
    );
